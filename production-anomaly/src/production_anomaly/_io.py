"""Input loading and column checks: a DataFrame, or a path to a .csv / .tsv / .parquet file."""
...
def load_table(source: TableLike, label: str = "df") -> pd.DataFrame:
    ...
    A DataFrame is returned as-is; callers copy before they change anything, so the
    caller's frame is never modified. ``label`` names the argument in error messages.
    """
        ...
        if ".parquet" in suffixes or ".pq" in suffixes:
            try:
                ...
                raise ImportError(
                    ...
                    'pip install "production-anomaly[parquet]"'
                ) from exc
        if ".csv" in suffixes:
    ...
    raise TypeError(
        f"{label} must be a pandas DataFrame or a path to a .csv/.tsv/.parquet file, "
        f"not {type(source).__name__}"
    )


def _read_separated(path: Path, label: str, sep: str, kind: str) -> pd.DataFrame:
    """Read a delimited text file with errors that name the argument and the file."""
    try:
        return pd.read_csv(path, sep=sep, encoding="utf-8-sig")
    except pd.errors.EmptyDataError:
        raise ValueError(f"{label}: {path.name} has no columns to read; the file is empty") from None
...
def check_columns(df: pd.DataFrame, label: str = "df") -> List[str]:
    ...
    if dupes:
        raise ValueError(
            ...
            "rename them so the time and output columns are unambiguous"
        )
    return names
